...
export default function SportsDayApp() {
  ...
  useEffect(() => {
    ...
    // 路徑使用您的專案結構
    const configRef = doc(db, 'sports-day', appId, 'data', 'config');
    ...
    const resultsRef = doc(db, 'sports-day', appId, 'data', 'results');
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800 pb-20">
      ...
      <main className="max-w-5xl mx-auto p-4">
        ...
        {currentView === 'admin_input' && isAdminMode && (
          <AdminInput config={config} results={results} />
        )}
...
function AdminSettings({ config }: { config: AppConfig }) {
    ...
    setLocalConfig((prev) => ({
      ...
      events: [
        ...prev.events,
        {
          ...
          type: eventType,
          gender: 'Mixed',
          unit: eventType === 'group' ? '名次' : '秒',
          sortBy: eventType === 'group' ? 'asc' : 'asc',
